...
//        die();
    ...
    public function actionAjaxHCDeliver()
    {
        $this->layout = false;
        $user = User::model()->findByPk(Yii::app()->user->getState('userid'));
    ...
    public function actionAjaxAdminReturn()
    {
        $this->layout = false;
        $user = User::model()->findByPk(Yii::app()->user->getState('userid'));
        $batchid = Yii::app()->request->getPost('batch_id');
    ...
    public function actionReport()
    {
        ...
        if(!empty($sd) && !empty($ed)){
            ...
            $payment = Payment::model()->reportPaymentByOsc($startdate,$enddate);
            ...
            $payment = Payment::model()->statPaymentByOsc();
            $ptrack = Passportscan::model()->statTrackingByOsc();
        }
        
        $admin = array();
        foreach($batch as $b){
            if(!in_array($b->branch_id,$admin[$b->admin_id])){
                $admin[$b->admin_id][] = $b->branch_id;
            }
        }
