...
class AdminController extends Controller
{
        ...
        return array(
            array('allow',
                'actions'=>array(
                    ...
                    'AjaxApplicantVerifyDeliver','AjaxApplicantVerifyReturn',
                    'AjaxOSCDeliver','AjaxBranchReturn',
    ...
    public function actionDashboard()
    {
        ...
        $batch1 = Batch::model()->statBatchByAdmin($user->admin_id,'today');
        $batch2 = Batch::model()->statBatchByAdmin($user->admin_id);
        
        $application1 = Applicant::model()->statApplicantByAdmin($user->admin_id,'today');
        $application2 = Applicant::model()->statApplicantByAdmin($user->admin_id);
        
        $payment1 = Payment::model()->statPaymentByAdmin($user->admin_id,'today');
        $payment2 = Payment::model()->statPaymentByAdmin($user->admin_id);
        
        $ptrack1 = Passportscan::model()->statTrackingByAdmin($user->admin_id,'today');
        $ptrack2 = Passportscan::model()->statTrackingByAdmin($user->admin_id);
    ...
    public function actionAjaxBarchartDaily()
    {
        ...
        $result = array(
            'admin' => $user->profile->company->name,
            'days' => $days,
            'visas' => $visa
        );
...
//        die();
    ...
    public function actionAjaxApplicantVerifyDeliver()
    {
        $this->layout = false;
        $user = User::model()->findByPk(Yii::app()->user->getState('userid'));
    ...
    public function actionAjaxOSCDeliver()
    {
        $this->layout = false;
        $user = User::model()->findByPk(Yii::app()->user->getState('userid'));
    ...
    public function actionAjaxBranchReturn()
    {
        $this->layout = false;
        $user = User::model()->findByPk(Yii::app()->user->getState('userid'));
        $batchid = Yii::app()->request->getPost('batch_id');
        $applicant = Yii::app()->request->getPost('applicant2');
        $result = array();
    ...
    public function actionReport()
    {
        ...
        if(!empty($sd) && !empty($ed)){
            ...
            $batch = Batch::model()->reportBatchByAdmin($user->admin_id,$startdate,$enddate);
            $application = Applicant::model()->reportApplicantByAdmin($user->admin_id,$startdate,$enddate);
            $payment = Payment::model()->reportPaymentByAdmin($user->admin_id,$startdate,$enddate);
            $ptrack = Passportscan::model()->reportTrackingByAdmin($user->admin_id,$startdate,$enddate);
        } else {
            $type = 'off';
            $batch = Batch::model()->statBatchByAdmin($user->admin_id);
            $application = Applicant::model()->statApplicantByAdmin($user->admin_id);
            $payment = Payment::model()->statPaymentByAdmin($user->admin_id);
            $ptrack = Passportscan::model()->statTrackingByAdmin($user->admin_id);
        }
        ...
        $report = array(
            ...
            'startformatdate' => $startdate,
            'endformatdate' => $enddate,
