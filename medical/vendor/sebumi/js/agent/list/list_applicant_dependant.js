...
$(document).on('click','.dependant-new',function() {
    $('#modal-new-dependant').modal('show');
    $('#form-new-dependant')[0].reset();
});

$(document).on('click','#register-dependant',function() {
    if($('#form-new-dependant').parsley().validate()){
        $.ajax({
            url: "index.php?r=Admin/AjaxDependantRegister",
            type: "POST",
            data: $('#form-new-dependant').serialize(),
            dataType: "JSON",
            success: function(data){
                if(data.status == 'success'){
                    ...
                    setTimeout(function(){
                        ...
                        $(location).attr('href','index.php?r=Admin/ListApplicantDependant&id=' + data.id);
                    }, 1000);
...
$(document).on('click','#update-dependant',function() {
    ...
    if($('#form-edit-dependant').parsley().validate()){
        $.ajax({
            url: "index.php?r=Staff/AjaxDependantUpdate&id=" + id,
            type: "POST",
