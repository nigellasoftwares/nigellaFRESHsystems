...
$(document).on('change','.number-dependant',function() {
    ...
    $('.tab-dependant').hide();
    $('#form-new-applicant').parsley().destroy();
    ...
    for(var i = 1; i < 6; i++){
        for(var j = 1; j < 10; j++){
            $('#form-required-dependant-' + i + '-' + j).removeAttr('data-required');
        }
    }
    ...
    if(dependant == 0){
        ...
        for(var i = 0; i < dependant; i++){
            ...
            for(var k = 1; k < 10; k++){
                $('#form-required-dependant-' + j + '-' + k).parsley({
                    required: true
                });        
            }
        }
...
$(document).on('click','#register-applicant',function() {
    if($('#form-new-applicant').parsley().validate()){
        ...
        */
        ...
        if($('select').hasClass('has-parsley-error')){
            $('a.chosen-single').css({
